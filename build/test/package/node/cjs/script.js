...
const checkCompose = () => {
  ...
  const aid = 'EFGDbi1kexM1ccTA_YSvTo9yW2eGx9BD6lmNNP5O-1Uo';
  const path = '/path/to/dids';
  const port = 8080;

  const did = didWebs.composeDid(hostname, aid, path, port);
  return (
    did ===
...
const checkPublish = () => {
  ...
  const [didJson, keriCesr, did] = didWebs.publishDid(cesr, {
    host: 'example.com',
    port: 8080,
    path: '/identity/did',
  });
  return (
    didWebs.compareDidDocuments(didJson, document) &&
    keriCesr === cesr &&
    did ===
      'did:webs:example.com%3A8080:identity:did:ECwJlFWWcXQRwMDP80dmDgEO949AqKOSR2sTGFli9aSc'
  );
