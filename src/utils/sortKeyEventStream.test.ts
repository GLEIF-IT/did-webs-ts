...
describe('sortEventStream', () => {
  ...
  it('should correctly sort events by AID in descending sequence order', () => {
    const events = parseKeyEventStream(cesr) as Event[];
    const sorted = sortKeyEventStream(events);
    // console.log(_fancyPrintSortedEvents(sorted, 1));

    // Expect three distinct AIDs
    expect(sorted.size).toBe(3);

    // Check AID: EKu2gkyoKhVsQBZEeYgp9SwGOl0PKnH4HvB10dkqC7lB
    const aid1 = sorted.get('EKu2gkyoKhVsQBZEeYgp9SwGOl0PKnH4HvB10dkqC7lB');
    expect(aid1).toBeDefined();
    if (aid1) {
      expect(aid1.length).toBe(1);
      expect((aid1[0] as { s: string }).s).toBe('0');
    }

    // Check AID: EFCwLHkNR2UAZrDHBd09iNzbzqdLy2hozJVL3swdd1dB
    const aid2 = sorted.get('EFCwLHkNR2UAZrDHBd09iNzbzqdLy2hozJVL3swdd1dB');
    expect(aid2).toBeDefined();
    if (aid2) {
      expect(aid2.length).toBe(2);
      expect((aid2[0] as { s: string }).s).toBe('1');
      expect((aid2[1] as { s: string }).s).toBe('0');
    }

    // Check AID: EBJ3FrFBhGxgfGNteaGONEPWCe5gt4V2d5qqsPGh3SsB
    const aid3 = sorted.get('EBJ3FrFBhGxgfGNteaGONEPWCe5gt4V2d5qqsPGh3SsB');
    expect(aid3).toBeDefined();
    if (aid3) {
      expect(aid3.length).toBe(1);
      expect((aid3[0] as { s: string }).s).toBe('0');
    }
  });
});
